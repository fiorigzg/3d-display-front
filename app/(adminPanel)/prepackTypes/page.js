...
export default function Home() {
  ...
  const header = [
    ...
    },
    {
      ...
      width: "50px",
      onClick: (ids) => {
        agreeMenu(`Точно удалить тип препака ${ids.id}?`, () =>
          projectsStore.deletePrepackType(ids.id),
        );
      },
    },
    {
    ...
    },
    {
      ...
      minWidth: "300px",
      onEnter: (ids, value) =>
        projectsStore.changePrepackType(ids.id, "name", value, "text", true),
    },
  ];
  ...
  return (
    <main>
      <div className={styles.table}>
        <HorizontalTable data={data} header={header} isLoading={projectsStore.isLoading} />
      </div>
    </main>
  );
}
